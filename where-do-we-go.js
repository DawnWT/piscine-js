...
export const explore = () => {
  ...
  document.addEventListener('scroll', e => {
    ...
    const sectionSize = document.documentElement.scrollHeight / sorted.length
    let n = 0
    while (window.scrollY + (window.innerHeight / 2) > sectionSize * (n + 1)) n += 1

    indicator.textContent = `${sorted[n].name}\n${sorted[n].coordinates}`
    indicator.style.color = sorted[n].color
    indicator.href = `https://www.google.com/maps/place/${sorted[n].coordinates}`
  })

  for (const place of sorted) {
    const section = document.createElement('section')
    section.style.background = `url('./where-do-we-go_images/${place.name.split(',')[0].split(' ').join('-').toLocaleLowerCase()}.jpg')`
